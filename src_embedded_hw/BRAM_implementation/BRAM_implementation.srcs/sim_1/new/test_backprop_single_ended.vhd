...
entity test_backprop_single_ended is
--  Port ( );
end test_backprop_single_ended;

architecture Behavioral of test_backprop_single_ended is
    ...
    constant iterations: natural := 5;
    
    constant nb_output_nodes: natural := sel(single_ended, 1, nb_classes);
    
    constant new_session: std_logic := '1';
    constant train: std_logic := '1';
    constant option: std_logic_vector(1 downto 0) := b"00";
...
--        (1795,-59,599,-1955);
    ...
    signal cnt_iter : natural range 0 to 200;
    signal disable_flag : boolean := true;
    signal cost_function : unsigned(15 downto 0) := (others => '0');
    
    signal new_epoch : boolean := false;
    signal cont_train_opcode, stop_train_opcode: std_logic_vector(31 downto 0) := (others => '0');
...
begin
    ...
    begin
        ...
        cont_train_opcode <= (19 => '0', 18 => '1', others => '0'); --15 downto 0 => std_logic_vector(to_unsigned(nb_examples,16)), 
        cont_train_opcode(15 downto 0) <= std_logic_vector(to_unsigned(nb_examples,16));
        
        stop_train_opcode <= (19 => '0', 18 => '0', others => '0');
        stop_train_opcode(15 downto 0) <= std_logic_vector(to_unsigned(nb_examples,16));
        ...
        S_TDATA(0) <= opcode;
        --S_TDATA(S_TDATA_length - 1) <= stop_train_opcode;
        ...
        for i in 0 to (nb_examples - 1) loop
            for j in 0 to (nb_features/4 - 1) loop
                S_TDATA(1 + ceil_div(weights1_RAM'length,2) + ceil_div(weights2_RAM'length,2) + i*ceil_div(nb_features+1,4) + j) 
            ...
            tmp := 1 + ceil_div(weights1_RAM'length,2) + ceil_div(weights2_RAM'length,2) + i*ceil_div(nb_features+1,4) + (nb_features/4);
            wait for 1 ns;
        ...
        while (true) loop
            if (AXIS_out.M_AXIS_TVALID = '1') then
                cost_function <= unsigned(Axis_out.M_AXIS_TDATA(15 downto 0));
                cnt_iter <= cnt_iter + 1;
                new_epoch <= true;         
            end if;
            
            wait for 0.01ns; -- udate new_epoch
            ...
            if(new_epoch) then
                ...
                if (cnt_iter < iterations) then
                    AXIS_in.S_AXIS_TDATA <= cont_train_opcode;
                    cnt_sent <= 1 + ceil_div((nb_features+1)*nb_hidden_nodes,2) + ceil_div((nb_hidden_nodes+1)*nb_output_nodes,2);
                else
                    exit;
                end if;
            else
        ...
        AXIS_in.S_AXIS_TDATA <= stop_train_opcode;
        while (true) loop
            wait for T_half; clk <= '1';
            wait for T_half; clk <= '0';
            if (AXIS_out.S_AXIS_TREADY = '1') then
