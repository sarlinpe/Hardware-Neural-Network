...
architecture implementation of main_controller is
    ...
    port (
        ...
        feedforward_out:        out feedforward_out_t;
        ...
        feedforward_out_avail:  out std_logic;
    ...
    --constant opcode_ses: natural := 18;
    --constant opcode_opt: natural := 16;
    ...
    signal feature_f, weight1_f, weight2_f : signed(11 downto 0);
    signal feedforward_out : feedforward_out_t (0 to (nb_output_nodes - 1));
    signal activ1_out : activ1_out_t(0 to nb_hidden_nodes-1);
    signal feature_f_read_req, weight1_f_read_req, weight2_f_read_req, raw_output_f_write_req : std_logic;
    ...
    signal weight1_f_addr : natural range 0 to ((nb_features+1)*nb_hidden_nodes);
    signal weight2_f_addr : natural range 0 to ((nb_hidden_nodes+1)*nb_output_nodes);
    ...
    signal feature_a, weight1_a, weight2_a : signed(11 downto 0); --, raw_output_a
    ...
    signal feature_a_write_req, label_a_write_req, weight1_a_write_req, weight2_a_write_req, 
           pred_a_read_req: std_logic; --raw_output_a_read_req, 
    ...
    --attribute mark_debug of main_state_curr, feedforward_en, backprop_en: signal is "true";
    --attribute mark_debug of feedforward_out_avail, feedforward_out, pred_done, pred_p, cnt_examples_out, backprop_not_ready, epoch_done: signal is "true";
    --attribute mark_debug of pred_a, pred_a_addr, pred_a_read_req: signal is "true";
    --attribute mark_debug of weight1_b_read, weight1_b_read_req, weight1_b_read_addr: signal is "true";
    --attribute mark_debug of weight2_b_read, weight2_b_read_req, weight2_b_read_addr: signal is "true";
    --attribute mark_debug of weight1_b_write, weight1_b_write_req, weight1_b_write_addr: signal is "true";
    --attribute mark_debug of weight2_b_write, weight2_b_write_req, weight2_b_write_addr: signal is "true";
    --attribute mark_debug of gen_req, gen_done, weight1_g, weight2_g, weight1_g_addr, weight2_g_addr, weight1_g_write_req, weight2_g_write_req: signal is "true";
...
--                weights1_f_RAM(weight1_a_addr) := weight1_a;
--                weights1_b_RAM(weight1_a_addr) := weight1_a;
--            elsif (weight1_g_write_req = '1') then
--                weights1_f_RAM(weight1_g_addr) := weight1_g;
--                weights1_b_RAM(weight1_g_addr) := weight1_g;
--            elsif (weight1_b_write_req = '1') then
--                weights1_f_RAM(weight1_b_write_addr) := weight1_b_write;
--                weights1_b_RAM(weight1_b_write_addr) := weight1_b_write;
--            end if;
            
--            if (weight2_a_write_req = '1') then
--                weights2_f_RAM(weight2_a_addr) := weight2_a;
--                weights2_b_RAM(weight2_a_addr) := weight2_a;
--            elsif (weight2_g_write_req = '1') then
--                weights2_f_RAM(weight2_g_addr) := weight2_g;
--                weights2_b_RAM(weight2_g_addr) := weight2_g;
--            elsif (weight1_b_write_req = '1') then
--                weights2_f_RAM(weight2_b_write_addr) := weight2_b_write;
--                weights2_b_RAM(weight2_b_write_addr) := weight2_b_write;
--            end if;
...
--            end if;
    ...
    BRAM_controller_read: process (clk) is
    begin
        if rising_edge(clk) then
            feature_f <= (others => '-');
            weight1_f <= (others => '-');
            ...
            if (weight1_f_read_req = '1') then
                weight1_f <= weights1_f_RAM(weight1_f_addr);
            end if;
            if (weight2_f_read_req = '1') then
                weight2_f <= weights2_f_RAM(weight2_f_addr);
            end if;
...
--                        if ( (feature_f_read_req = '1') and (cnt_ex_block_outdated /= nb_virtual_blocks) ) then -- if last example then no need to update next block
--                            if ( cnt_ex_block_curr >= (cnt_ex_block_outdated - 1) ) then -- action needed
--                                if ( cnt_ex_block_curr = cnt_ex_block_outdated ) then
--                                    feedforward_en <= '0';
--                                end if;
--                                if (update_requested = '0') then
--                                    update_requested_next <= '1';
