...
entity feedforward_dataflow is
    generic (
        ...
        delay_sigmoid:          natural := 2 
    );
    port (
        feature :               in signed(11 downto 0);
        
        weight1 :               in weights1_bus_t(0 to nb_hidden_nodes-1);
        weight2 :               in weights2_bus_t(0 to nb_output_nodes-1);
        hidden_layer_out :      out activ1_out_t;
        raw_output :            out feedforward_out_t;

        --cnt_s1_hidden :         in natural range 0 to nb_hidden_nodes; -- useless
        cnt_s3_hidden :         in natural range 0 to nb_hidden_nodes;
        --cnt_s3_out :            in natural range 0 to (nb_output_nodes - 1); -- useless
...
architecture version1 of feedforward_dataflow is
    ...
    -- Stage 1 registers
    type prod_s1_t is array (0 to (nb_hidden_nodes-1)) of signed(23 downto 0);
    signal prod_s1, buf_prod_s1: prod_s1_t;
    signal buf_weight_s1: weights1_bus_t(0 to nb_hidden_nodes-1);
    signal buf_feat_s1: signed(11 downto 0);
    type sum_s1_t is array (0 to (nb_hidden_nodes - 1)) of signed(31 downto 0);
    signal sum_s1: sum_s1_t := (others => (others => '0'));
    
    --signal idx_sum1_tmp1, idx_sum1_tmp2, idx_sum1_tmp3 : natural range 0 to (nb_hidden_nodes - 1) := 0; -- maybe useless
    ...
    -- Stage 3 registers
    type prod_s2_t is array (0 to (nb_output_nodes-1)) of signed(23 downto 0);
    signal prod_s3, buf_prod_s3: prod_s2_t;
    signal buf_weight_s3: weights2_bus_t(0 to nb_output_nodes-1);
    signal buf_feat_s3: signed(11 downto 0);
    type sum_s3_t is array (0 to (nb_output_nodes - 1)) of signed(31 downto 0);
    signal sum_s3: sum_s3_t;
    
    --signal idx_sum3_tmp1, idx_sum3_tmp2, idx_sum3_tmp3 : natural range 0 to (nb_output_nodes - 1) := 0;
        ...
        if rising_edge(clk) then
            if(feedforward_en = '1') then
                ...
                if (stage1_rst = '1') then
                    ...
                    buf_weight_s1 <= (others => (others => '0'));
                    buf_feat_s1 <= (others => '0');
                    prod_s1 <= (others => (others => '0'));
                    buf_prod_s1 <= (others => (others => '0'));
                    ...
                    for i in 0 to (nb_hidden_nodes-1) loop
                        prod_s1(i) <= buf_feat_s1 * buf_weight_s1(i);
                        sum_s1(i) <= sum_s1(i) + buf_prod_s1(i);
                    end loop;
                    buf_prod_s1 <= prod_s1;
        ...
        if rising_edge(clk) then
            if (feedforward_en = '1') then
                ...
                if (stage3_rst = '1') then
                    -- Reset all internal registers
                    buf_weight_s3 <= (others => (others => '0'));
                    buf_feat_s3 <= (others => '0');
                    prod_s3 <= (others => (others => '0'));
                    buf_prod_s3 <= (others => (others => '0'));
                    ...
                    if (cnt_s3_hidden = 0) then
                        buf_feat_s3 <= to_signed(1,12); -- bias values
                    else
                        buf_feat_s3 <= latch_hidden_layer(cnt_s3_hidden - 1);
                    end if;
                    for i in 0 to (nb_output_nodes-1) loop
                        prod_s3(i) <= buf_weight_s3(i) * buf_feat_s3;
                        sum_s3(i) <= sum_s3(i) + buf_prod_s3(i);
                    end loop;
                    buf_prod_s3 <= prod_s3;
