...
architecture version1 of feedforward_controller is
    ...
    constant delay_stage1 : natural := 3;
    constant delay_stage3 : natural := 3;
...
begin
    ...
    stage1_combi_proc: process (state1_curr, cnt_examples_in, cnt_s1_features,cnt_s1_delay, --cnt_s1_hidden, 
    ...
    begin
        ...
        feature_addr_idx <= 0; --
        feature_read_req <= '0';
        weight1_addr <= (others => 0);
        weight1_read_req <= (others => '0');
        stage1_done <= '0';
...
--                    end if;
            ...
            when Accumulate =>
                ...
                for i in 0 to (nb_hidden_nodes-1) loop
                    weight1_read_req(i) <= '1';
                    weight1_addr(i) <= cnt_s1_features * nb_hidden_nodes + i; --cnt_s1_hidden;
                end loop;
                ...
                if (true) then --(cnt_s1_hidden = (nb_hidden_nodes - 1)) then -- move to another feature
    ...
    begin
        if rising_edge(clk) then
            if (feedforward_en = '1') then
                ...
                --cnt_hidden <= cnt_hidden_next;
            end if;
        end if;
    end process stage3_fsm_proc;
    
    stage3_combi_proc: process (state3_curr, cnt_s3_hidden, cnt_s3_delay, --cnt_s3_out, 
                                feedforward_en, weight2_ram, stage2_done) is
    begin
        stage3_rst <= '0';
        weight2_addr <= (others => 0);
        weight2_read_req <= (others => '0');
        stage3_done <= '0';
...
--                    end if;
            ...
            when Accumulate =>
                ...
                for i in 0 to (nb_output_nodes-1) loop
                    weight2_read_req(i) <= '1';
                    weight2_addr(i) <= cnt_s3_hidden * nb_output_nodes + i; --cnt_s3_out;
                end loop;
                
                if (true) then --(cnt_s3_out = (nb_output_nodes - 1)) then
