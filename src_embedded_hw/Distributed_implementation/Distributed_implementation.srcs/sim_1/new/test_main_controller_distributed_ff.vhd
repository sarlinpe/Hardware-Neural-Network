...
entity test_main_controller_distributed_ff is
--  Port ( );
end test_main_controller_distributed_ff;

architecture Behavioral of test_main_controller_distributed_ff is
    ...
    constant nb_features: natural := 4;
    ...
    constant nb_examples : natural range 0 to (nb_examples_max-1) := 4;
...
begin
    ...
    begin
        ...
        for i in 0 to (weights1_RAM'length - 1) loop
            weights1_RAM(i) <= -2000 + 250*(i+1) ;--2*i-2000;
        end loop;
        for i in 0 to (weights2_RAM'length - 1) loop
            weights2_RAM(i) <= 500*(i+1) + 1;--90*i+1;
        end loop;
        wait for 1 ns;
        ...
        for i in 0 to (nb_examples - 1) loop
            for j in 0 to ((nb_features+1)/4 - 1) loop
            ...
            tmp := 1 + offset + i*ceil_div(nb_features+1,4) + ((nb_features+1)/4);
            wait for 1 ns;
